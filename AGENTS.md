...
- **WiFi 状态图标（Kindle 端叠加，与横幅同机制）**：画面角落常显 40×40 扇形图标——`ifconfig wlan0` 有 inet addr → 实心；无 → 斜杠。与 wget 成败组合：斜杠+横幅=断网；实心+横幅=服务器/远端问题。图标永远最后绘制，不被横幅遮盖。位置随方向：竖屏=左上（x44,y14）；横屏两种方向都在画面右上角（landscape-cw 用帧缓冲 x44,y14、landscape-ccw 用 x988,y1394），图标文件按方向预旋转（`wifi-*-cw/ccw.png`）。
