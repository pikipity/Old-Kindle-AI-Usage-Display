...
6. 配置完成后，设备会停在以下三种界面**之一**，都属于正常：
   - "配置设备（CONFIGURE DEVICE）"界面；
   - 提示"展示机无内容或未连接网络"的界面；
   - **宣传壁纸轮番切换**（卖场展示循环）——最常见的状态。
   
   不管是哪种，都需要用**秘密手势**跳过（壁纸轮播的话，直接在壁纸上做手势即可）：
...
2. 连电脑，把与你 Kindle 界面语言对应的 hotfix 拷到根目录。界面是简体中文就选 `Update_hotfix_languagebreak-zh-Hans-CN.bin`（美式英语是 `-en-US`，以此类推）。
   - 此时根目录**没有** `mkk` 文件夹是正常的——它在 hotfix 装完后才会出现，别为此返工。
3. 弹出盘符、拔线。搜索框输入 `;dsts` 按回车进入设置页，【更新您的 Kindle（Update your Kindle）】在**设置页右上角 ⋮（三个点）菜单里**，点它并确认。
   - 该项是灰色点不动 = 没找到有效更新包：检查 .bin 确实在根目录、文件名没有 `(1)` 之类后缀。
...
1. 连电脑看 Kindle 根目录，应该多了一个 **`mkk` 文件夹**（越狱密钥落地的标志，hotfix 装完才出现）。
2. 在 Kindle 搜索框输入 `;log` 按回车，屏幕出现 **`No arg passed. select from mrpi or runme`** 之类的提示文字——这句就是越狱命令钩子生效的标志（它同时告诉你：`;log mrpi` 是后面装插件要用的命令）。
...
> ⚠ **Managed 模式修复的副作用（实测确认）**：上面的修复动作会**清空用户存储区**——`mkk`、`libkh`、`rp` 文件夹和 `documents` 里的内容都会被删掉（所以第一节的备份很重要）。修复完成后按下面顺序恢复：
>
> 1. 先确认系统层越狱还在：搜索框输 `;log`，能看到提示文字就说明钩子还在（Managed 修复不会动系统层）。
> 2. 把 hotfix 的 `.bin` 重新拷到根目录，【设置 → ⋮ → 更新您的 Kindle】重装一遍，重启后 `mkk` 就回来了。
> 3. 确认 `mkk` + `;log` 双验证通过，再继续第四节。
...
>
> 注意：个别设备上 PEKI 拷完后图书馆里**始终不出现 KUAL 图标**（内容索引问题，`mkk` 正常也没用）。遇到这种情况和下面"备选"路线的取舍很简单：先花两分钟按第七节对照表排查，不行就直接走备选。

### 4.2（备选）用 MRPI 安装 KUAL

如果 PEKI 版拷完图书馆里一直不出现 KUAL 图标，或图标点开白屏/闪退，换这条传统路线：
...
| **装好 hotfix 之后**仍没有 `mkk` 文件夹 / `;log` 无输出 | 越狱没成功：常见是拷成了 `LanguageBreak` 文件夹本身（多套了一层），或第 3.3-6 步插线太慢错过时间窗口（注意：hotfix 装完之前没有 `mkk` 是正常的，别因此返工） | 恢复出厂后重做 3.2～3.3，注意"拷内容不拷文件夹"，且按钮示意图一出现就立刻插线 |
...
| 修复 Managed 模式后 `mkk`/`libkh`/`rp` 不见了 | 修复动作会**清空用户存储区**，属已知副作用 | 系统层越狱不受影响：先 `;log` 验证钩子还在，再重装一遍 hotfix 恢复 `mkk`，详见 3.5 节 |
| PEKI 拷完 `KUAL.sh`/`KUAL.jar`，图书馆里没有 KUAL 图标 | 内容没被索引；下载的是 Source code 源码包；个别设备 PEKI 路线就是不显示 | ① 搜索框直接搜 `KUAL`；② 图书馆筛选里勾上"个人文档"；③ 确认解压的是 `PEKI.zip` 且 `KUAL.jar` 是几十 KB 以上的二进制；④ 都不行就换 4.2 备选（MRPI）路线，它不依赖内容索引 |
